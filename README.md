...
xf-webvjは、Web上で動作するVJアプリケーションです。2つのデッキを持ち、指定されたフォルダに格納されたローカル動画ファイル、またはYouTubeの埋め込み動画をロードできます。

バックエンドでyt-dlpを実行する事で、アプリケーション内でYoutubeやniconicoのURLから動画を直接ダウンロード出来ます。

## 主な特徴
- 2つの独立したデッキによる動画再生
- ローカル動画ファイルとYouTube埋め込み再生の両対応
- デッキ間のスムーズなクロスフェード
- 再生速度の調整（BPM計算機能付き）
- 各デッキの透明度制御
- 動画のダウンロードと管理
...
4. 「動画リスト」タブからローカル動画を選ぶか、YouTube URL を各デッキへ直接ロードします
